...
const BookForm = (props) => {
  ...
  const addNewBook = async () => {
    ...
    const parsedBookRecord = {
      ...bookRecord,
      pageCount: parseInt(bookRecord.pageCount)
    }
    ...
    try {
      const response = await fetch("/api/v1/books", {
        ...
        body: JSON.stringify(parsedBookRecord)
      })
      if (!response.ok) {
  ...
  if (shouldRedirect) {
    return <Redirect push to={`/books/${redirectId}`} />
  }
  console.log(errors);
